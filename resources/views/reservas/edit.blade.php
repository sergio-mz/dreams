...
<div class="container">
    ...
    <form action="{{ route('planes.update', $plane) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="number" name="price" id="price" class="form-control" step="0.01" value="{{ old('price', $plane->price) }}">
            @error('price')
                <small class="text-danger">{{ $message }}</small>
            @enderror
        </div>
            ...
            <div>
                @foreach($domos as $domo)
                    <label class="domo-label">
                        <input type="checkbox" name="domos[]" value="{{ $domo->id }}"
        ...
        <button type="submit" class="btn btn-primary">Actualizar plane</button>
    </form>
</div>
